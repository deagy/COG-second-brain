...
- **A doc comment describes the API's contract, not your caller.** `loader.ForExtension`'s comment said callers could register a loader via `DirectoryLoader.Loaders`, and that was read twice as evidence a chat-export loader had a seam there — the CLI's `loaderForPath` passes `nil` for that field and routes single files around `DirectoryLoader` entirely. The quote was accurate; the inference from it was not. Grep the actual call site before writing "X is the seam for Y" into a note.
