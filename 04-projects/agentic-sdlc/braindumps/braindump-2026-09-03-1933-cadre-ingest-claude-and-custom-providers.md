...
against that hook without touching the built-in dispatch.~~ **Wrong — see the correction
below.** The quote is accurate; the conclusion drawn from it is not. The hook serves library
consumers of the `loader` package, and `recall upload` is not one: it passes `nil` for
`Loaders` and routes single files around `DirectoryLoader` entirely. I read the doc comment
and did not check its only caller.

The design questions are about mapping, not plumbing: does one export become one document
or one document per conversation; do turns become chunks or get flattened; what
conversation metadata (title, create time, model, role) survives into document metadata so
it is filterable at retrieval.
...
> **Correction, 2026-09-03 (writing SPEC-002):** the `DirectoryLoader.Loaders`
> claim below and in "What remains" is wrong for the CLI. `loaderForPath` in
> `cmd/recall/local.go` passes `nil` for `Loaders` on the directory path, and sends
> single files to `loader.ForExtension(ext)`, bypassing `DirectoryLoader` entirely.
> Registering a loader against that hook changes nothing for `recall upload`; the
> work is in `loaderForPath` and `ForExtension`. The hook is real for library
> consumers of the `loader` package, which is where I read it — I did not check
> whether the CLI used it.
> [Source: `~/sdk/recall/cmd/recall/local.go:27-37` | 2026-09-03 | confidence: high]
...
Specified in [[spec-chat-export-loader]] (SPEC-002), which resolved two of the
three open questions by finding the constraint that decides them:

- The loader is wired into `loaderForPath`, **not** `DirectoryLoader.Loaders` — see
  the correction above.
- **Sniffing, not a `--format` flag.** `Loaders` is keyed by extension so there can
  be only one `.json` loader, and directory ingest passes no per-file format, so a
  flag cannot express a directory holding both export kinds.
- **The role goes in the chunk text, not metadata.** One Document per conversation
  means the loader cannot attach per-turn metadata at all — chunk metadata comes
  from the chunker. Prefixing each turn with its speaker is the only way the role
  reaches the chunk, and it gets embedded rather than sitting in a field retrieval
  may not surface. `MetaSectionIndex` remains unusable as a turn number (raw split
  index, empties skipped, count computed pre-drop).
...
- [ ] Build the chat-export loader per [[spec-chat-export-loader]] — wired into `loaderForPath`, not `DirectoryLoader.Loaders` 📅 2026-09-10
