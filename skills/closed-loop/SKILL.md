...
- **Fold in the run's approvals.** If `<run-dir>/evidence/approvals.tsv` has rows (written by `checkpoint.sh record_approval` at the moment each external mutation was approved, in Phase 6), copy each into the `human_approvals` array of the `lifecycle_gates` entry whose `gate_id` matches the row's gate. `human_approvals` is a property of a gate object, never of the run-record root — the root is `additionalProperties: false` and rejects a top-level one. The ledger is the record; this fold makes the run-record a report assembled from it, which is why it happens here and not at the moment of the mutation.
