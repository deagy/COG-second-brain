...
`read_only` is the hard case: the specialist may inspect but must not write. Under
claude-code the tier is enforced natively — the named agent is generated with exactly
the tools its tier allows (`runner-capabilities.json`, `capability_tiers`), and a
`PreToolUse` hook can deny anything that slips past. Under codex, where dispatch falls
back to `spawn_agent` with per-file `developer_instructions`, there is no native gate:
state the restriction as a behavioural contract in the task — the role reads and
reports, never edits — and treat a write as a sandbox violation.

## 4. DISPATCH

Spawn the specialist as a sub-agent with a single, bounded task. The specialist
writes its output to a file and returns only a short status + path — never the full
output inline.

Dispatch by **name**: `runner-capabilities.json` records
`named_agent_dispatch_supported: true` for claude-code, with the naming
`agents:<role-id>` when the roster is plugin-installed and a bare `<role-id>` for a
project-local override. The generated agent already carries the role's `AGENT.md` as
its system prompt and its tier's tools, so pass only the task:

```
Agent(subagent_type="<role-id>", prompt="<bounded task; name the output file to write>")
```

If the role is not installed as a named agent, fall back to a generic sub-agent and
paste the role's `AGENT.md` (step 2) in as its system prompt verbatim — do not
paraphrase it. `spawn_agent(systemPrompt=..., task=...)` is the codex fallback path,
not claude-code's.
