...
**A session can ship a great deal without ever being a harness run, and that is not a loophole.** One session reviewed a branch across four passes, withdrew two of its four changes, cut three releases across three repositories and propagated them to two importers — with no lane, no spec and no gate, because none was asked for. What substituted was review passes, mutation checks and artifact verification, all chosen rather than enforced. If that is the wrong outcome the trigger below is too narrow; if it is the right one, the harness's scope is genuinely narrower than its length suggests. The question is recorded here rather than answered, so the next session can decide it instead of re-deriving it.
